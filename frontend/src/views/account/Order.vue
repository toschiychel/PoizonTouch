                    ...
                    <div class="col-lg-3">
                        <div class="wrap-sidebar-account">
                            <ul class="my-account-nav">
                                <li>
                                    <router-link :to="{ name: 'account'}" class="my-account-nav-item">Главная</router-link>
                                </li>
                                <li>
                                    <router-link :to="{ name: 'account.order'}" class="my-account-nav-item active">Заказы</router-link>
                                </li>
                                <!-- <li><a href="my-account-address.html" class="my-account-nav-item">Адресс</a></li> -->
                                <li>
                                    <router-link :to="{ name: 'account.edit'}" class="my-account-nav-item">Информация о вас</router-link>
                                </li>
                                <!-- <li><a href="my-account-wishlist.html" class="my-account-nav-item">Добавленный товар</a></li> -->
                                <li><a @click.prevent="logout()" href="#" class="my-account-nav-item">Выйти</a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="col-lg-9">
                        <template v-if="orders">
                            <div v-if="orders.length == 0" class="page-404-wrap">
                                <div class="container">
                                    <div class="row">
                                        <div class="col-12">
                                            <div class="title">
                                            ...
                                            <router-link :to="{ name: 'product.index' }" class="tf-btn btn-sm radius-3 btn-fill btn-icon animate-hover-btn">К покупкам</router-link>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </template>
                        <div v-for="order in orders" :key="order.id" class="wd-form-order mb-3">
                            <div class="order-head">
                                <!-- <figure class="img-product">
                                    <img :src="order.preview_image" alt="product">
                                </figure> -->
                                <div class="content">
                                    <h6 class="mt-8 fw-6">Заказ #{{order.id}} </h6>
                                    <div class="badge">{{ order.status}}</div>
                                </div>
                            </div>
                            <div class="tf-grid-layout md-col-2 gap-15">
                                <div class="item">
                                    <div class="text-2 text_black-2">Заказ создан</div>
                                    <div class="text-2 mt_4 fw-6 mb-3">{{ order.created_at }}</div>
                                    <div class="text-2 text_black-2">Имя получателя</div>
                                    <div class="text-2 mt_4 fw-6">{{ order.contacts.last_name}} {{ order.contacts.first_name }}</div>
                                </div>
                                <div class="item">
                                    <div class="text-2 text_black-2">Адрес</div>
                                    <div class="text-2 mt_4 fw-6 mb-3">{{ order.contacts.address }}</div>
                                    <div class="text-2 text_black-2">Номер телефона</div>
                                    <div class="text-2 mt_4 fw-6 mb-3">{{ order.contacts.phone }}</div>
                                </div>
                            </div>
                            <div class="widget-tabs style-has-border widget-order-tab">
                                <ul class="widget-menu-tab">
                                    <li :class="['item-title', { active: activeTabs[order.id] === 'details' }]" @click="setActiveTab(order.id, 'details')">
                                        <span class="inner">Детали заказа</span>
                                    </li>
                                    <li v-if="order.delivery" :class="['item-title', { active: activeTabs[order.id] === 'delivery' }]" @click="setActiveTab(order.id, 'delivery')">
                                        <span class="inner">Доставка</span>
                                    </li>
                                </ul>
                                <div class="widget-content-tab">
                                    <div :class="['widget-content-inner', { active: activeTabs[order.id] === 'details' }]">
                                        <div v-for="item in order.positions" class="order-head">
                                            <figure class="img-product">
                                                <img :src="item.preview_image" alt="product">
                                            </figure>
                                            <div v-if="item.type == 'link'" class="content">
                                                <div class="text-2 fw-6">{{ item.title}}</div>
                                                <div class="mt_4"><span class="fw-6">Ссылка на товар:</span> {{ item.link_url}}</div>
                                                <div class="mt_4"><span class="fw-6">Кол-во :</span> {{ item.quantity}}</div>
                                                <div class="mt_4"><span class="fw-6">Цена(шт) :</span> {{ item.price_cny }}¥</div>
                                            </div>
                                            <div v-if="item.type == 'product'" class="content">
                                                <div class="text-2 fw-6">{{ item.title}}</div>
                                                <div class="mt_4"><span class="fw-6">Кол-во :</span> {{ item.quantity}}</div>
                                                <div class="mt_4"><span class="fw-6">Цена(шт) :</span> {{ item.unit_price}}₽</div>
                                            </div>
                                            <div v-if="item.type == 'delivery'" class="content">
                                                <div class="text-2 fw-6">{{ item.title}}</div>
                                                <div class="mt_4"><span class="fw-6">Цена:</span> {{ item.unit_price}}₽</div>
                                            </div>
                                        </div>
                                        <ul>
                                            <li v-if="order.status == 'Ожидает оплаты'" class="d-flex justify-content-between text-2 mt_4 pb_8 line">
                                                <button @click.prevent="createPayment(order.id)" class="tf-btn btn-fill animate-hover-btn btn-address mb_20">Оплатить заказ</button>
                                            </li>
                                            <li class="d-flex justify-content-between text-2 mt_8">
                                                <span v-if="order.status == 'Ожидает подсчёта цены'" class="fw-6">Ориентировочная сумма:</span>
                                                <span v-if="order.status !== 'Ожидает подсчёта цены'" class="fw-6">Общая сумма:</span>

                                                <span class="fw-6">{{ order.total_price ? order.total_price : 'X'}}₽</span>
                                            </li>

                                        </ul>
                                    </div>
                                    <div v-if="order.delivery" :class="['widget-content-inner', { active: activeTabs[order.id] === 'delivery' }]">
                                        <div class="widget-timeline">
                                            <ul class="timeline">
                                                <li v-for="event in order.delivery.events">
                                                    <div class="timeline-badge success"></div>
                                                    <div class="timeline-box">
                                                        <a class="timeline-panel" href="javascript:void(0);">
                                                            <div class="text-2 fw-6">{{ event.description }}</div>
                                                            <span>{{ event.happened_at }}</span>
                                                        </a>
                                                        <p v-if="event.location"><strong>Местонахождение : </strong>{{ event.location }}</p>
                                                    </div>
                                                </li>
                                            </ul>
                                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="wrap">
                <div class="tf-product-media-wrap">
                    <img class="p-5" v-if="qr" :src="qr" alt="Telegram QR Code" />
                </div>
                <div class="tf-product-info-wrap position-relative">
                    <div class="tf-product-info-list text-center">
                        <div class="tf-product-info-title">
                            <h5>Включите оповещения</h5>
                        </div>
                        <div class="badges text-uppercase">Отсканируйте QrCode</div>
                        <div class="badges text-uppercase">Отправьте боту /start</div>
                        <div class="badges text-uppercase">Дождитесь подтверждения привязки в ответном сообщении</div>
                        <div class="badges text-uppercase">Теперь вы получаете оповещения в телеграм</div>
                    </div>
                </div>
            </div>
        </div>
...
<a v-show="false"  ref="tgBtn" @click="fetchQr" href="#quick_view" data-bs-toggle="modal" class="tf-btn w-25 radius-3 btn-fill animate-hover-btn justify-content-center" :class="{ 'highlight': shouldHighlight }">
    Добавить Telegram
</a>
</template>
    ...
    methods: {
        ...
        getOrders() {
            ...
            axios.get('/sanctum/csrf-cookie', {
                ...
                .then(response => {
                    axios.get(`/api/account/${id}/orders`)
                        .then(res => {
                            this.orders = res.data.data
                            this.orders.forEach(order => {
                                this.activeTabs[order.id] = 'details';
                            });
                        })
                })
        },

        setActiveTab(orderId, tab) {
            // Простое назначение ключа в Vue 3 срабатывает реактивно
            this.activeTabs[orderId] = tab;
        },
        ...
        createPayment(orderId) {
            axios.post(`/api/order/${orderId}/createPayment`)
                .then(res => {
            ...
            axios.get('/sanctum/csrf-cookie', {
                ...
                .then(res => {
                    axios.get('/api/telegram/qr', {
                        ...
                        .then(response => {
                            // Получаем qr-строку из response.data
                            this.qr = response.data.qr;
                        })
                        .catch(err => {
                            console.error('Ошибка при получении QR:', err);
                            this.closeModal();
                            alert('Не удалось получить QR-код. Проверьте авторизацию и правильность URL.');
                        });
                })
        },
