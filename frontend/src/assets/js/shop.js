...
(function ($) {
  ...
  var rangeTwoPrice = function () {
      ...
      var defaultMin = 1, defaultMax = 100;
      var rawMin = parseInt(skipSlider.getAttribute("data-min"), 10);
      var rawMax = parseInt(skipSlider.getAttribute("data-max"), 10);

      var min = isNaN(rawMin) ? defaultMin : rawMin;
      var max = isNaN(rawMax) ? defaultMax : rawMax;
      if (min === max) {
        min = defaultMin;
        max = defaultMax;
      }
    ...
    function applyFilters() {
      ...
      if (visibleProductCountGrid >= 1 || visibleProductCountList >= 1) {
